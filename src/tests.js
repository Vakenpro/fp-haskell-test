...
const Tests = () => {
    ...
    const handleSubmit = () => {
        trueAnswers === testdata.length ? alert("Все верно, вы гений") 
   ...
   <ul>
       {testdata.map((test)=>(
        <li key={test.id}>{test.question}<br/>
        <form>
            {test.answers.map((answer)=>(
            <label key={answer.id}>
                <input type="radio" name={test.id} value={answer.text} onClick={()=>handleChoose(test.id-1 /*, answer.text */, answer.id)}/>
                {answer.text}
            </label>
            ))}
        </form>
